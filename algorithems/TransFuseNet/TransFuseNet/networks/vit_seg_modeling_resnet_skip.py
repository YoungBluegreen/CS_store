...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.root(x)    # 12 64 112 112  第一层特征图
        features.append(x)
        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=0)(x)         # 12 64 55 55
        for i in range(len(self.body) - 1):     # self.body=3
            x = self.body[i](x)             # 12 256 55 55   12 512 28 28
