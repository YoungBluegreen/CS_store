    ...
    <div class="flex-row">
      ...
      <a-modal :visible="sVisible"
         ...
         <a-form :rules="rules" ref="formRef" :model="uploadParam" :label-col="{ span: 6 }">
          ...
          <a-form-item name="device_name" label="适用设备" required>
            <a-select
              style="width: 220px"
              mode="multiple"
              placeholder="可选择多个设备型号"
...
const deviceNameList = ref<any[]>([{ label: '全部设备', value: '' }])
...
const rules = {
  ...
  device_name: [{ required: true, message: '请选择该固件适用的设备型号。' }]
}
interface FileItem {
  uid: string;
  name?: string;
...
function beforeUpload (file: FileItem) {
  if (!file.name || !file.name?.endsWith('.zip')) {
    message.error('文件格式错误，请选择 zip 固件包。')
    return false
  }
  fileList.value = [file]
  return false
    ...
    importFirmareFile(workspaceId, fileData).then((res) => {
      if (res.code === 0) {
        notification.success({
          ...
          description: `[${file.name}] 文件上传成功，用时：${moment.duration(new Date().getTime() - timestamp).asSeconds()} 秒`,
          duration: null
        })
        getAllFirmwares(pageParam)
      } else {
        notification.error({
          message: `[${file.name}] 上传失败，请检查后重试。`,
...
function formatFirmwareStatus (status: string) {
