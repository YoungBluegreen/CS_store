<template>
    ...
    <div class="content">
      <a-form ref="valueRef" layout="horizontal" :hideRequiredMark="true" :rules="rules" :model="planBody" labelAlign="left">
        ...
        <a-form-item label="飞行航线" :wrapperCol="{offset: 7}" name="file_id">
          <a @click.prevent="openWaypointPlanner">选择航线</a>
        </a-form-item>
        ...
        <!-- 设备 -->
        <a-form-item label="执行设备" :wrapperCol="{offset: 10}" v-model:value="planBody.dock_sn" name="dock_sn">
          <router-link
            :to="{name: 'select-plan'}"
            @click="selectDevice"
          >选择设备</router-link>
        ...
        <a-form-item v-if="planBody.dock_sn" style="margin-top: -15px;">
          <div class="panel" style="padding-top: 5px;">
            ...
            <div class="ml10 mt5" style="color: hsla(0,0%,100%,0.65);">
              <span><RocketOutlined /></span>
              <span class="ml5">{{ dock.children?.nickname ?? '未绑定无人机' }}</span>
            </div>
          </div>
        </a-form-item>
        <!-- 任务类型 -->
        ...
        <template v-if="planBody.task_type === TaskType.Condition">
          <!-- battery capacity -->
          <a-form-item label="电量达到以下阈值后执行" :labelCol="{span: 23}" name="min_battery_capacity">
          ...
          <!-- storage capacity -->
          <a-form-item label="存储剩余达到以下阈值后执行(MB)" :labelCol="{span: 23}" name="storage_capacity">
            <a-input-number v-model:value="planBody.min_storage_capacity" class="width-100">
            </a-input-number>
          </a-form-item>
        </template>
        <!-- RTH Altitude Relative to Dock -->
        <a-form-item label="相对机场返航高度(m)" :labelCol="{span: 23}" name="rth_altitude">
  ...
  <div v-if="drawerVisible" class="task-side-panel" :class="{ planner: routeName === 'WaylinePanel' }">
    <div>
...
const rules = {
  name: [
    ...
    { max: 20, message: '长度需在 1 到 20 个字符之间' }
  ],
...
function openWaypointPlanner () {
  drawerVisible.value = true
  routeName.value = 'WaylinePanel'
  root.$router.push({ name: ERouterName.SELECT_PLAN })
}
