      ...
      <div class="box">
        ...
        <div class="row">
          ...
          <Button type="primary" size="small" danger ghost @click="handleEmergencyStop" >
            <template #icon><PauseCircleOutlined/></template><span>急停</span>
          </Button>
        </div>
        <div class="row">
          <DroneControlPopover
          ...
          >
            <template #formContent>
              <div class="form-content">
                ...
                <div>
                  <span class="form-label">指令飞行失控动作：</span>
                  <a-select
                ...
                <div>
                  <span class="form-label">指令飞行模式：</span>
                  <a-select
                ...
                <div>
                  <span class="form-label">指令飞行高度(m)：</span>
...
async function sendControlCmd (cmdItem: DeviceCmdItem, index: number) {
  ...
  if (result) {
    message.success('返航指令发送成功')
    if (flightController.value) {
      exitFlightCOntrol()
    }
  } else {
    message.error('返航指令发送失败')
  }
