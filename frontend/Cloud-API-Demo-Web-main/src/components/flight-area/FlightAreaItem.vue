<template>
  <div class="panel" style="padding-top: 5px;" :class="{disable: !flightArea.status}">
    ...
    <div class="mt5 ml10" style="color: hsla(0,0%,100%,0.35);">
      <span class="mr10">更新时间 {{ formatDateTime(flightArea.update_time).toLocaleString() }}</span>
    </div>
    <div class="flex-row flex-justify-between flex-align-center ml10 mt5" style="color: hsla(0,0%,100%,0.65);">
      ...
      <div class="mr10 operate">
        <a-popconfirm v-if="flightArea.status" title="确认停用当前区域？" okText="停用" cancelText="取消" @confirm="changeAreaStatus(false)">
          <stop-outlined />
        </a-popconfirm>
        <a-popconfirm v-else @confirm="changeAreaStatus(true)" title="确认启用当前区域？" okText="启用" cancelText="取消" >
          <check-circle-outlined />
        </a-popconfirm>
        <EnvironmentFilled class="ml10" @click="clickLocation"/>
        <a-popconfirm title="确认删除当前区域？" okText="删除" cancelText="取消" okType="danger" @confirm="deleteArea">
