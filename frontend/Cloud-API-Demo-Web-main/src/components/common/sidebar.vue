<template>
  <div class="demo-project-sidebar-wrapper flex-justify-between">
    ...
    <div class="mb20 flex-display flex-column flex-align-center flex-justify-between">
      <a-tooltip title="返回主页" placement="right">
...
export default defineComponent({
  ...
  setup () {
    const root = getRoot()
    const options = [
      { key: 0, label: '飞行态势', path: '/' + ERouterName.TSA, icon: 'TeamOutlined' },
      { key: 1, label: '实时直播', path: '/' + ERouterName.LIVESTREAM, icon: 'VideoCameraOutlined' },
      { key: 2, label: '图层标注', path: '/' + ERouterName.LAYER, icon: 'EnvironmentOutlined' },
