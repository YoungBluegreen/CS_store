<template>
  <div class="flex-column flex-justify-start flex-align-center">
    ...
    <p class="fz16 mt10">
      提示：以下参数可从 https://console.agora.io 获取
    </p>
...
const onUpdateQuality = () => {
  ...
  setLivestreamQuality({
    ...
    if (res.code === 0) {
      message.success('清晰度已切换为' + clarityList[dronePara.claritySelected].label)
    }
  })
}
...
const onSwitch = () => {
  if (dronePara.lensSelected === undefined || dronePara.lensSelected === nonSwitchable) {
    message.info('当前镜头不可切换，请选择可切换镜头', 8)
    return
  }
